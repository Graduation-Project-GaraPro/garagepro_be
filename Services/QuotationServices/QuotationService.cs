...
namespace Services.QuotationServices
{
    public class QuotationManagementService : IQuotationService
    {
        ...
        public async Task<QuotationDto> ProcessCustomerResponseAsync(CustomerQuotationResponseDto responseDto)
        {
            ...
            // Update service selection (QuotationServices)
            if (responseDto.SelectedServices != null && responseDto.SelectedServices.Any())
            {
                ...
                {
                    // Customer can select services (but required services must stay selected)
                    if (qs.IsRequired)
                    {
                        qs.IsSelected = true; // Required services cannot be deselected
                    }
                    else
                    {
            ...
            {
                // If no services sent, keep required services selected, deselect optional ones
                foreach (var qs in quotation.QuotationServices)
                {
                    qs.IsSelected = qs.IsRequired;
                }
            }

            // Update part selection (QuotationServiceParts) - Customer selects which parts they want
            if (responseDto.SelectedServices != null)
            {
                foreach (var selectedService in responseDto.SelectedServices)
                {
                    ...
                    if (quotationService != null && selectedService.SelectedParts != null)
                    {
                        // Create a set of selected part IDs for this service
                        var selectedPartIds = selectedService.SelectedParts
                    ...
                    else if (quotationService != null)
                    {
                        // If no parts specified, deselect all parts for this service
                        foreach (var part in quotationService.QuotationServiceParts)
                        {
